...
public class CloudPendingInvoicesDataStore extends RestBase implements InvoicesDataStore {

    private static final String TAG = CloudPendingInvoicesDataStore.class.getSimpleName();
     ...
     * @since 06/12/2018
     */
    public CloudPendingInvoicesDataStore(Context poContext) {
        super(poContext);
    }
    ...
    public void getPendingInvoices(CallbackDataStore<List<PendingInvoicesEntity>> poCallback, String token, RequestPendingInvoicesEntity poRequest) {
        LogUtil.i(TAG, "INICIO - Obtener Recibos Pendientes");
        String lsContext = getContext().getString(R.string.context_multipago);
        String lsWs = getContext().getString(R.string.ws_recibos);
        String lsEndpoint = getContext().getString(R.string.endpoint_lista_recibos_deudas_nis);
        String lsUrl = String.format("%s%s%s", lsContext, lsWs, lsEndpoint);

        Call<BaseResponseEntity<List<PendingInvoicesEntity>>> loCall =
                getRestApi().getListPendingInvoices(lsUrl, token,poRequest);

        Log.i("URL Obtenida", loCall.request().url().toString());

        RestReceptor<List<PendingInvoicesEntity>> loRestReceptor = new RestReceptor<List<PendingInvoicesEntity>>(getContext());

        loRestReceptor.invoke(loCall, new RestCallback<List<PendingInvoicesEntity>>() {
            @Override
            public void onResponse(List<PendingInvoicesEntity> poData, BaseResponseEntity poBaseResponseEntity) {
                LogUtil.i(TAG, "FIN - Obtener Recibos Pendientes: onResponse");
                poCallback.onSuccess(poData);
            }

            @Override
            public void onError(BaseException poException) {
                LogUtil.i(TAG, "FIN - Obtener Recibos Pendientes: onError");
