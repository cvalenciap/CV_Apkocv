...
public class CloudValidPayRefDataStore extends RestBase implements ValidPayRefDataStore {

    private static final String TAG = CloudValidPayRefDataStore.class.getSimpleName();
     ...
     * @since 06/12/2018
     */
    public CloudValidPayRefDataStore(Context poContext) {
        super(poContext);
    }

    @Override
    public void valid(CallbackDataStore<String> poCallback, String token, RequestPayRefEntity poRequest) {
        LogUtil.i(TAG, "INICIO - Valida Referencia cobro");
        ...
        String lsEndpoint = getContext().getString(R.string.endpoint_valida_ref_cobro);
        String lsUrl = String.format("%s%s%s", lsContext, lsWs, lsEndpoint);

        Call<BaseResponseEntity<String>> loCall =
                getRestApi().validPaymentReferency(lsUrl, token,poRequest);
