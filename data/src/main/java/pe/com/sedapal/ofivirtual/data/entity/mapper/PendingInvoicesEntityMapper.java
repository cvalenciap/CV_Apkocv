 ...
 */
@Singleton
public class PendingInvoicesEntityMapper {

    /**
     * Constructs a {@link PendingInvoicesEntityMapper}.
     *
     * @author jsaenz
     * @version 1.0
     * @since 06/12/2018
     ...
     * @since 07/12/2019
     */
    public PendingInvoices mapToEntity(PendingInvoicesEntity poUserEntity) {
        PendingInvoices loUser = null;

        if (poUserEntity != null) {
            loUser = ParseUtil.parseObject(poUserEntity, PendingInvoices.class);
     ...
     * @since 07/12/2019
     */
    public List<PendingInvoices> mapToEntity(List<PendingInvoicesEntity> poListDocumentTypeEntity) {

        final List<PendingInvoices> loDocumentType;
        ...
        if (poListDocumentTypeEntity != null && !poListDocumentTypeEntity.isEmpty()) {
            ...
            for (PendingInvoicesEntity loPendingInvoicesTypeEntity : poListDocumentTypeEntity) {
                final PendingInvoices loPendingInvoicesDto = mapToEntity(loPendingInvoicesTypeEntity);
                if (loPendingInvoicesDto != null) {
                    loDocumentType.add(loPendingInvoicesDto);
                }
            }
        } else {
